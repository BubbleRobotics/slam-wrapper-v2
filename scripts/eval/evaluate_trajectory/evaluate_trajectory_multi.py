...
class TrajectoryEvalMulti:
    ...
    def relative_error(self):
        ...
        for evaluator in self.evaluators:
            ...
            def fill_in_pos_vec(row, rel_err):
                """Function applied to df to fill in error data from
                different pipeline runs"""
                # For the given row, identify the subtrajectory length and fill in from
                # the correct field of the relative error named tuple
                
                # Retrieve the right field in the relative error tuple.
                idx = self.cfg.rte.trajec_lengths.index(row["subtraj_len"])
                current_pos_err_vec = row["pos_err_vec"]
                return np.r_[current_pos_err_vec, rel_err[idx].pos_vec]

            def fill_in_rot_vec(row, rel_err):
                """Function applied to df to fill in error data from
                different pipeline runs"""
                # For the given row, identify the subtrajectory length and fill in from
                # the correct field of the relative error named tuple
                ...
                # Retrieve the right field in the relative error tuple.
                idx = self.cfg.rte.trajec_lengths.index(row["subtraj_len"])
                current_pos_err_vec = row["rot_err_vec"]
                return np.r_[current_pos_err_vec, rel_err[idx].rot_vec]
            ...
            if self.rte_df is None or not (self.rte_df["gt_name"]==evaluator.seq_name).any():
                ...
                for i, tup in enumerate(rel_err):
                    ...
                    new_rows.append(
                        {"gt_name": evaluator.seq_name,
                         ...
                         "pos_err": tup.pos,    # L2 norm relative position error
                         "rot_err": tup.rot,    # L2 norm relative rotation error
                         "pos_err_vec": tup.pos_vec,   # complete relative position error vectors
                         "rot_err_vec": tup.rot_vec,   # complete relative rotation error vectors
                ...
                self.rte_df.loc[self.rte_df["gt_name"]==evaluator.seq_name, "pos_err_vec"] \
                    = self.rte_df.loc[self.rte_df["gt_name"]==evaluator.seq_name].apply(
                        fill_in_pos_vec, args=(rel_err,), axis=1)
                
                self.rte_df.loc[self.rte_df["gt_name"]==evaluator.seq_name, "rot_err_vec"] \
                    = self.rte_df.loc[self.rte_df["gt_name"]==evaluator.seq_name].apply(
                        fill_in_rot_vec, args=(rel_err,), axis=1)
